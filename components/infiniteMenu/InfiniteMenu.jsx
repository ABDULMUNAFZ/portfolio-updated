...
/* ── Rounded Rectangle Geometry (replaces DiscGeometry) ── */
class RoundedRectGeometry extends Geometry {
  constructor(width = 1.6, height = 1.0, cornerRadius = 0.12, cornerSegments = 8) {
    super();
    const hw = width / 2;
    const hh = height / 2;
    ...
    // 4 corners, each with an arc
    const corners = [
      { cx: hw - r, cy: hh - r, sa: 0, ea: Math.PI / 2 },
      { cx: -(hw - r), cy: hh - r, sa: Math.PI / 2, ea: Math.PI },
      { cx: -(hw - r), cy: -(hh - r), sa: Math.PI, ea: 1.5 * Math.PI },
      { cx: hw - r, cy: -(hh - r), sa: 1.5 * Math.PI, ea: 2 * Math.PI },
    ];

    for (const c of corners) {
      for (let i = 0; i <= cornerSegments; i++) {
        const angle = c.sa + ((c.ea - c.sa) * i) / cornerSegments;
        const x = c.cx + r * Math.cos(angle);
        const y = c.cy + r * Math.sin(angle);
        this.addVertex(x, y, 0);
        this.lastVertex.uv[0] = (x / hw) * 0.5 + 0.5;
        this.lastVertex.uv[1] = (y / hh) * 0.5 + 0.5;
      }
    }

    // Fan triangulation from center
    const perimCount = (cornerSegments + 1) * 4;
    for (let i = 1; i < perimCount; i++) this.addFace(0, i, i + 1);
    this.addFace(0, perimCount, 1);
  }
}
...
class InfiniteGridMenu {
  ...
  #init(onInit) {
    ...
    // Use rounded rectangle geometry instead of disc
    this.discGeo = new RoundedRectGeometry(1.6, 1.0, 0.12, 8);
  ...
  #initTexture() {
    ...
    // Load images or generate styled cards
    const imagePromises = this.items.map(item => {
      if (item.image) {
    ...
    Promise.all(imagePromises).then(images => {
      images.forEach((img, i) => {
        ...
        if (img) {
          ...
          this.#drawCertCard(ctx, x, y, cellSize, this.items[i], i);
        }
      });
  ...
  #drawCertCard(ctx, x, y, size, item, index) {
    const pad = 20;
    ...
    // Subtle inner radial glow — no border
    const innerGrad = ctx.createRadialGradient(x + size / 2, y + size * 0.35, 0, x + size / 2, y + size / 2, size * 0.5);
    innerGrad.addColorStop(0, 'rgba(71, 103, 232, 0.08)');
    innerGrad.addColorStop(0.6, 'rgba(246, 142, 212, 0.03)');
    innerGrad.addColorStop(1, 'rgba(0, 0, 0, 0)');
    ctx.fillStyle = innerGrad;
    ctx.fillRect(x, y, size, size);

    // Large number watermark
    ctx.fillStyle = 'rgba(222, 244, 56, 0.05)';
    ctx.font = 'bold 200px sans-serif';
    ctx.textAlign = 'right';
    ctx.fillText(`${(index + 1).toString().padStart(2, '0')}`, x + size - pad - 10, y + size - pad - 10);

    // Decorative top accent line
    const accentGrad = ctx.createLinearGradient(x + size * 0.2, y + pad, x + size * 0.8, y + pad);
    accentGrad.addColorStop(0, 'rgba(71, 103, 232, 0)');
    accentGrad.addColorStop(0.5, 'rgba(71, 103, 232, 0.4)');
    accentGrad.addColorStop(1, 'rgba(246, 142, 212, 0)');
    ...
    for (const word of words) {
      ...
      if (metrics.width > size - pad * 4 && currentLine) {
    ...
    const lineHeight = 52;
    const titleStartY = y + size * 0.42;
    lines.forEach((line, i) => {
      ctx.fillText(line, x + size / 2, titleStartY + i * lineHeight);
    });

    // Description (pink)
    ctx.fillStyle = '#f68ed4';
    ctx.font = '22px sans-serif';
    const descY = titleStartY + lines.length * lineHeight + 24;
    ...
    for (const w of descWords) {
      ...
      if (ctx.measureText(test).width > size - pad * 6 && descLine) {
    ...
    descLines.forEach((dl, i) => {
      ctx.fillText(dl, x + size / 2, descY + i * 28);
    });
...
export default function InfiniteMenu({ items = [], scale = 1.0 }) {
  ...
  useEffect(() => {
    ...
    const sketch = new InfiniteGridMenu(
      canvas,
      items.length ? items : [{ image: '', title: 'Certificate', description: '' }],
      handleActiveItem,
      handleMovement,
      sk => sk.run(),
      scale
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      <div className={`drag-hint ${showHint ? 'visible' : 'hidden'}`}>
        ...
        <span>Click and drag to see more</span>
      </div>
